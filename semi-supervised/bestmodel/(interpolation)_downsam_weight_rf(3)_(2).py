...
def train_tcn_with_confidence(df1, df2, df3, true_labels_df3, timesteps=16, confidence_threshold=0.80):
    """
    Train a TCN model using labeled data and Self-Training with Random Forest.
    """
    ...
    df1_labeled, df1_unlabeled = train_test_split(df1, test_size=0.97, random_state=42, stratify=df1['fms_3class'])
    df2_labeled, df2_unlabeled = train_test_split(df2, test_size=0.97, random_state=42, stratify=df2['fms_3class'])
    df3_labeled, df3_unlabeled = train_test_split(df3, test_size=0.97, random_state=42, stratify=df3['fms_3class'])
