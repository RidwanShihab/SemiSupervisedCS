...
from tensorflow.keras.layers import Conv1D, MaxPooling1D, BatchNormalization, Dropout, Flatten, Reshape, LSTM
...
def train_tcn_with_confidence(df1, df2, df3, true_labels_df3, timesteps=16, confidence_threshold=0.80):
    ...
    print(class_weights)
    #  **Modify Data Shapes for CNN-LSTM**
    num_features = X_labeled.shape[1]  # Total number of features

    X_train_high_conf = X_train_high_conf.reshape(-1, timesteps, num_features // timesteps)  # Reshape for CNN
    ...
    model = Sequential([
        ...
        Dense(3, activation='softmax')  # Match the TCN output layer for classification
    ])
