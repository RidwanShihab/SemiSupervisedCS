...
from keras.layers import Dense, Conv1D, MaxPooling1D, BatchNormalization, Dropout, Flatten, Reshape, LSTM
...
def train_cnn_lstm_with_confidence(df1, df2, df3, true_labels_df3, timesteps=16, confidence_threshold=0.80):
    """
    Train a CNN-LSTM model using labeled data and Self-Training with Random Forest.
    """
    # Split data randomly instead of by participants
    ...
    def extract_features_labels(df_labeled, df_unlabeled):
        ...
        y_unlabeled = df_unlabeled['fms_3class'].values
    ...
    # Apply Self-Training with Random Forest
    base_rf = RandomForestClassifier(n_estimators=100, random_state=42)
    self_training_model = SelfTrainingClassifier(base_rf, criterion="k_best", k_best=50)
    self_training_model.fit(np.vstack((X_labeled, X_unlabeled)), np.concatenate((y_labeled, [-1] * len(X_unlabeled))))

    # Get pseudo-labels for unlabeled data
    transduced_labels = self_training_model.predict(np.vstack((X_labeled, X_unlabeled)))

    # Check true labels vs. predicted labels for unlabeled data
    print("Classification Report for Unlabeled Data (Self-Training Random Forest):")
    print(classification_report(y_unlabeled, transduced_labels[len(y_labeled):]))

    cm = confusion_matrix(y_unlabeled,  transduced_labels[len(y_labeled):])
    ...
    # Select high-confidence samples for TCN training
    high_confidence_indices = np.where(transduced_labels[len(y_labeled):] != -1)[0]
    X_train_high_conf = X_unlabeled[high_confidence_indices]
    y_train_high_conf = transduced_labels[len(y_labeled):][high_confidence_indices]

    class_weights = compute_class_weight(class_weight='balanced', classes=np.unique(y_train_high_conf), y=y_train_high_conf)
    class_weight_dict = dict(enumerate(class_weights))

    num_features = X_labeled.shape[1]
    X_train_high_conf = X_train_high_conf.reshape(-1, timesteps, num_features // timesteps)
    X_test = X_test.reshape(-1, timesteps, num_features // timesteps)

    X_train, X_val, y_train, y_val = train_test_split(X_train_high_conf, y_train_high_conf, test_size=0.2, random_state=42, stratify=y_train_high_conf)

    model = Sequential([
        Conv1D(filters=64, kernel_size=3, activation='relu', input_shape=(timesteps, num_features // timesteps)),
        BatchNormalization(),
    ...
    return model, history, y_test, yhat_classes, yhat_probs, testdf1, testdf2, testdf3, self_training_model




true_labels_df3 = sams_scaled['fms_3class'].values
model, history, y_test_labeled, yhat_classes, yhat_probs,testdf1,testdf2,testdf3,label_propagation = train_cnn_lstm_with_confidence(df_scaled, d2_scaled, sams_scaled, true_labels_df3)
