...
def build_model(input_shape, head_size=256, num_heads=4, ff_dim=4, num_transformer_blocks=4, mlp_units=[128, 64], dropout=0.2, mlp_dropout=0.3):
...
def train_transformer_with_confidence(df1, df2, df3, true_labels_df3, timesteps=16, confidence_threshold=0.80):
    ...
    class_weightslp = compute_class_weight(class_weight='balanced', classes=np.unique(y_labeled), y=y_labeled)
...
model, history, y_test_labeled, yhat_classes, yhat_probs,testdf1,testdf2,testdf3,label_propagation = train_transformer_with_confidence(df_scaled, d2_scaled, sams_scaled, true_labels_df3)
