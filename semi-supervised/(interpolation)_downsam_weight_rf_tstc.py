...
def positional_encoding(seq_len, d_model):
    """Generates positional encoding for a given sequence length."""
...
def transformer_encoder(inputs, head_size, num_heads, ff_dim, dropout=0):
    """Improved Transformer block for Time Series"""
    pos_encoding = positional_encoding(inputs.shape[1], inputs.shape[2])
    inputs += pos_encoding  # Adding positional encoding
...
def build_model(
    ...
    x = layers.Flatten()(x)  # Use Flatten instead of GlobalPooling
...
def train_tcn_with_confidence(df1, df2, df3, true_labels_df3, timesteps=16, confidence_threshold=0.80):
    ...
    model = build_model(
        ...
        ff_dim=8,  # Increase feed-forward network size
        num_transformer_blocks=6,  # Increase depth
        mlp_units=[256, 128, 64],  # Deeper MLP layers
        mlp_dropout=0.3,
        dropout=0.25,
    )
...
yhat_proba = yhat_probs

# Generate Precision-Recall Curve for each class
plt.figure(figsize=(10, 8))
for i in range(3):  # Assuming 3 classes
    precision, recall, _ = precision_recall_curve(y_test_labeled == i, yhat_proba[:, i])
