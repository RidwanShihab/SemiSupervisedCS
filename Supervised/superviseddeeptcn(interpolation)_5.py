...
def train_tcn_with_confidence(df1, df2, df3, true_labels_df3, timesteps=16):
    """
    Train a TCN model using labeled data only.
    """
    df1, _ = train_test_split(df1, test_size=0.95, random_state=42, stratify=df1['fms_3class'])
    df2, _ = train_test_split(df2, test_size=0.95, random_state=42, stratify=df2['fms_3class'])
    df3, _ = train_test_split(df3, test_size=0.95, random_state=42, stratify=df3['fms_3class'])
    ...
    dfs = [df1, df2, df3, testdf1, testdf2, testdf3]
    for df in dfs:
    ...
    X_labeled = np.concatenate((df1.drop(columns=['fms_3class']).values,
                                 df2.drop(columns=['fms_3class']).values,
                                 df3.drop(columns=['fms_3class']).values), axis=0)
    y_labeled = np.concatenate((df1['fms_3class'].values,
                                 df2['fms_3class'].values,
                                 df3['fms_3class'].values), axis=0)

    # Compute class weights
    class_weights = compute_class_weight(class_weight='balanced', classes=np.unique(y_labeled), y=y_labeled)
    class_weight_dict = dict(enumerate(class_weights))

    # Prepare data for TCN
    num_features = X_labeled.shape[1]
    X_labeled = X_labeled.reshape(-1, timesteps, num_features // timesteps)
    ...
    return model, history, y_test, yhat_classes, yhat_probs, testdf1, testdf2, testdf3

true_labels_df3 = sams_scaled['fms_3class'].values
model, history, y_test_labeled, yhat_classes, yhat_probs, testdf1, testdf2, testdf3 = train_tcn_with_confidence(df_scaled, d2_scaled, sams_scaled, true_labels_df3)
